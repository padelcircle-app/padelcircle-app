...
def append_rows(neu: pd.DataFrame, sheet_name: str,
                key_cols: list = None, id_spalte: str = None) -> int:
    """
    ...
    def kennung(df):
        """Echte ID wenn brauchbar, sonst Zeilen-Hash."""
    ...
    if not alt.empty:
        alt = alt.copy()
        if "_hash" not in alt.columns:
            alt["_hash"] = _zeilen_hash(alt, gemeinsam)
        vorhanden = set(kennung(alt).astype(str))
        neu = neu[~neu["_id"].astype(str).isin(vorhanden)]
        if neu.empty:
            return 0
        gesamt = pd.concat([alt, neu.drop(columns=["_id"])], ignore_index=True)
    else:
        gesamt = neu.drop(columns=["_id"])
...
def fuzzy_match(suchname: str, kandidaten: list, mapping: dict,
    ...
    for kand in frei:
        ...
        score = min(100.0, token * 0.5 + partial * 0.2 + laut * 0.2 + bonus)
        if score > 50:
...
def _dash_tag():
    ...
    with c2:
        gewaehlt = st.selectbox(
            "Tag", tage, index=st.session_state.tag_idx,
            format_func=datum_lang, label_visibility="collapsed",
            key="tag_wahl")
        if gewaehlt != tage[st.session_state.tag_idx]:
            st.session_state.tag_idx = tage.index(gewaehlt)
            st.rerun()
                    ...
                    with u2:
                        if st.button("Wieder öffnen", key=f"dt_undo_{i}_{datum}",
                            ...
                            behebung_zuruecknehmen(str(e["name_norm"]),
                                                   str(e["datum"]))
                            cache_leeren()
                            st.success("Fall ist wieder offen.")
                            time.sleep(.9)
                            st.rerun()
...
def _wa_nachverfolgung():
    """Fälle mit verschickter Nachricht — hat jemand nachgeholt?"""
    tage = verfuegbare_tage()
    if not tage:
        box("Noch keine Daten.", "warn")
        return

    fenster = CONFIG["nachhol_fenster_tage"]
    box(f"Nach deiner Nachricht holen viele den Check-in nach. Die App sucht "
        f"in den <b>{fenster} Tagen</b> nach dem Spiel nach passenden Check-ins. "
        "Bestätigst du eine Nachholung, ist der Fall erledigt und der Check-in "
        "gilt als verbraucht — er taucht nicht mehr als überzählig auf.", "info")

    offen = faelle_mit_nachricht(tage)
    if offen.empty:
        box("✅ Keine offenen Fälle.", "ok")
        return

    angeschrieben = offen[offen["_tage_seit"] >= 0]
    unberuehrt = offen[offen["_tage_seit"] < 0]

    # Nachhol-Kandidaten suchen
    treffer = []
    for _, r in angeschrieben.iterrows():
        k = nachhol_kandidaten(str(r["Name"]), str(r["Datum"]))
        # Nur Check-ins NACH dem Spieltag zählen als Nachholung
        k = [x for x in k if x[4] > 0]
        if k:
            treffer.append((r, k[0]))

    c1, c2, c3 = st.columns(3)
    with c1:
        kpi("Angeschrieben", str(len(angeschrieben)), "warten auf Reaktion")
    with c2:
        kpi("Nachgeholt", str(len(treffer)), "bereit zum Abhaken")
    with c3:
        ueberfaellig = int((angeschrieben["_tage_seit"] > fenster).sum())
        kpi("Frist abgelaufen", str(ueberfaellig),
            f"älter als {fenster} Tage")

    # ── Erkannte Nachholungen ───────────────────────────────────────────
    if treffer:
        st.markdown("---")
        st.markdown("##### Vermutlich nachgeholt")
        box(f"<b>{len(treffer)} Spieler</b> haben nach deiner Nachricht "
            "eingecheckt. Ein Klick und beide Seiten sind erledigt.", "ok")

        if len(treffer) > 1:
            if st.button(f"✓ Alle {len(treffer)} Nachholungen bestätigen",
                         type="primary", use_container_width=True):
                for r, k in treffer:
                    nachholung_speichern(k[2], k[1], str(r["Datum"]),
                                         str(r["Name_norm"]))
                cache_leeren()
                st.success(f"{len(treffer)} Fälle geschlossen.")
                st.balloons()
                time.sleep(1.3)
                st.rerun()
            st.markdown("")

        for i, (r, k) in enumerate(treffer):
            anzeige, kand_norm, ci_datum, score, tage_danach = k
            wann = "am nächsten Tag" if tage_danach == 1 else f"{tage_danach} Tage später"
            st.markdown(
                f'<div class="pc-card"><div class="pc-fall">'
                f'<div><span class="nm">{r["Name"]}</span>'
                f'<span class="mt">&nbsp;· gespielt {datum_kurz(str(r["Datum"]))}'
                f' · angeschrieben vor {int(r["_tage_seit"])} Tagen</span></div>'
                f'</div></div>', unsafe_allow_html=True)
            st.markdown(
                f'<div class="pc-vorschlag">✓ <b>{anzeige}</b> hat am '
                f'<b>{datum_kurz(ci_datum)}</b> eingecheckt — {wann}. '
                f'{chip(f"{score:.0f}%", "lime" if score >= 85 else "warn")}</div>',
                unsafe_allow_html=True)

            n1, n2, n3 = st.columns([1.3, 1, 2])
            with n1:
                if st.button("✓ Nachgeholt", key=f"nh_ok_{i}", type="primary",
                             use_container_width=True):
                    nachholung_speichern(ci_datum, kand_norm,
                                         str(r["Datum"]), str(r["Name_norm"]))
                    cache_leeren()
                    st.success("Fall geschlossen.")
                    time.sleep(.9)
                    st.rerun()
            with n2:
                if st.button("✗ Passt nicht", key=f"nh_no_{i}",
                             use_container_width=True):
                    rejected_speichern(str(r["Name_norm"]), kand_norm)
                    st.rerun()
            with n3:
                st.caption(f"Check-in am {datum_lang(ci_datum)}")
            st.markdown("")

    # ── Warten auf Reaktion ─────────────────────────────────────────────
    ohne_treffer = [r for _, r in angeschrieben.iterrows()
                    if not any(t[0]["Name_norm"] == r["Name_norm"]
                               and t[0]["Datum"] == r["Datum"] for t in treffer)]
    if ohne_treffer:
        st.markdown("---")
        st.markdown("##### Angeschrieben, noch kein Check-in")
        st.caption(f"{len(ohne_treffer)} Fälle · nach {fenster} Tagen läuft "
                   "die Frist ab")

        for i, r in enumerate(ohne_treffer[:40]):
            seit = int(r["_tage_seit"])
            abgelaufen = seit > fenster
            farbe = "err" if abgelaufen else ("warn" if seit >= fenster - 1 else "soft")
            status = ("Frist abgelaufen" if abgelaufen
                      else f"noch {fenster - seit} Tage")

            w1, w2, w3 = st.columns([2.4, 1.4, 1.2])
            with w1:
                st.markdown(f"**{r['Name']}** · gespielt {datum_kurz(str(r['Datum']))}")
                st.caption(f"angeschrieben vor {seit} "
                           + ("Tag" if seit == 1 else "Tagen"))
            with w2:
                st.markdown(chip(status, farbe), unsafe_allow_html=True)
            with w3:
                if st.button("Erledigt", key=f"nh_erl_{i}",
                             use_container_width=True):
                    als_behoben_markieren(str(r["Name_norm"]), str(r["Datum"]))
                    cache_leeren()
                    st.rerun()
    ...
    if not unberuehrt.empty:
        st.markdown("---")
        box(f"<b>{len(unberuehrt)} Fälle</b> wurden noch nicht angeschrieben — "
            "die findest du im Tab <i>Offene Fälle</i>.", "info")


def _wa_ueberzaehlig():
    """Check-ins, die zu keiner Buchung passen — manuell zuordnen."""
    tage = verfuegbare_tage()
    if not tage:
        box("Noch keine Daten.", "warn")
        return

    box("Diese Personen haben eingecheckt, ohne dass eine passende Buchung "
        "vorliegt. Meist ist es eine Namensvariante oder eine Nachholung für "
        "einen früheren Tag. Hier kannst du beides von Hand zuordnen.", "info")

    zeitraum = st.radio("Zeitraum", ["7 Tage", "30 Tage", "Alles"],
                        horizontal=True, key="uez_zeit")
    grenze = {"7 Tage": 7, "30 Tage": 30, "Alles": len(tage)}[zeitraum]
    ziel_tage = tage[:grenze]

    von = parse_date_safe(ziel_tage[-1])
    bis = parse_date_safe(ziel_tage[0])
    if von is None or bis is None:
        box("Zeitraum nicht lesbar.", "warn")
        return

    offen = offene_checkins_zeitraum(von, bis)
    if offen.empty:
        box("✅ Alle Check-ins sind zugeordnet.", "ok")
        return

    c1, c2 = st.columns(2)
    with c1:
        kpi("Überzählige Check-ins", str(len(offen)))
    with c2:
        kpi("Wert", euro(len(offen) * WELLPASS_WERT),
            "bereits von EGYM vergütet")

    st.markdown("---")

    # Alle offenen Fälle als Zuordnungsziele
    alle_faelle = alle_offenen_fehler(ziel_tage)

    for i, (_, r) in enumerate(offen.head(50).iterrows()):
        ...
        st.markdown(
            f'<div class="pc-card"><div class="pc-fall">'
            f'<div><span class="nm">{ci_name}</span>'
            f'<span class="mt">&nbsp;· eingecheckt {datum_kurz(ci_datum)}'
            + (f' um {ci_zeit}' if ci_zeit else '') + '</span></div>'
            '</div></div>', unsafe_allow_html=True)

        if alle_faelle.empty:
            st.caption("Keine offenen Fälle zum Zuordnen.")
            continue

        # Passende Fälle vorschlagen: gleicher oder früherer Spieltag
        ci_d = parse_date_safe(ci_datum)
        kandidaten = []
        for _, f in alle_faelle.iterrows():
            f_d = parse_date_safe(str(f["Datum"]))
            if f_d is None or ci_d is None:
                continue
            abstand = (ci_d - f_d).days
            if not (0 <= abstand <= CONFIG["nachhol_fenster_tage"]):
                continue
            sc = fuzz.token_set_ratio(normalize_name(str(f["Name"])), ci_norm)
            mail = email_fuer(str(f["Name"]))
            if mail:
                sc = max(sc, email_aehnlichkeit(mail, ci_name))
            kandidaten.append((sc, abstand, str(f["Name"]),
                               str(f["Name_norm"]), str(f["Datum"])))
        kandidaten.sort(key=lambda x: (-x[0], x[1]))

        if not kandidaten:
            st.caption("Kein passender Fall im Zeitfenster gefunden.")
            st.markdown("")
            continue

        beschriftung = {}
        for sc, abstand, nm, nn, dt in kandidaten[:25]:
            wann = "gleicher Tag" if abstand == 0 else f"{abstand} Tage vorher"
            beschriftung[f"{nm}  ·  {datum_kurz(dt)} ({wann})  ·  {sc:.0f}%"] = (nn, dt)

        wahl = st.selectbox("Zu welchem Fall gehört dieser Check-in?",
                            ["—"] + list(beschriftung.keys()),
                            key=f"uez_sel_{i}")
        if wahl != "—":
            nn, dt = beschriftung[wahl]
            if st.button("✓ Zuordnen und Fall schließen", key=f"uez_ok_{i}",
                         type="primary", use_container_width=True):
                nachholung_speichern(ci_datum, ci_norm, dt, nn)
                cache_leeren()
...
def modul_whatsapp():
    ...
    t1, t2, t3, t4, t5 = st.tabs([
        "📬 Offene Fälle", "🔄 Nachverfolgung", "👀 Überzählige Check-ins",
        "✅ Erledigt", "📜 Protokoll"])
    ...
    with t1:
        ...
        # ── Einzelfälle ─────────────────────────────────────────────────
        st.markdown("---")
        # ── Fälle sortieren: erst die mit Zuordnungsvorschlag ───────────
        offen = offen.copy()
        offen["_score"] = 0.0
        offen["_vorschlag"] = ""
        for idx, r in offen.iterrows():
            v = zuordnung_vorschlag(str(r["Name"]), str(r["Datum"]),
                                    mail=email_fuer(str(r["Name"])),
                                    zeit=str(r.get("Service_Zeit", "")))
            if v:
                offen.at[idx, "_score"] = v[0][2]
                offen.at[idx, "_vorschlag"] = v[0][0]

        mit_vorschlag = offen[offen["_score"] >= 70]
        ohne_vorschlag = offen[offen["_score"] < 70]

        f1, f2, f3 = st.columns(3)
        with f1:
            kpi("Offene Fälle", str(len(offen)))
        with f2:
            kpi("Vermutlich Namensvariante", str(len(mit_vorschlag)),
                "schnell zu klären")
        with f3:
            kpi("Echte Vergesser", str(len(ohne_vorschlag)),
                "anschreiben oder nachmelden")

        st.markdown("")
        ansicht = st.radio(
            "Anzeigen",
            [f"Namensvarianten ({len(mit_vorschlag)})",
             f"Echte Vergesser ({len(ohne_vorschlag)})",
             f"Alle ({len(offen)})"],
            horizontal=True, label_visibility="collapsed")

        if ansicht.startswith("Namensvarianten"):
            zeige = mit_vorschlag
            if not zeige.empty:
                box("Diese Personen haben eingecheckt — nur unter einem anderen "
                    "Namen. Ein Klick auf <b>Ist dieselbe</b> klärt den Fall "
                    "dauerhaft.", "ok")
        elif ansicht.startswith("Echte Vergesser"):
            zeige = ohne_vorschlag
            if not zeige.empty:
                box("Für diese Personen gibt es keinen passenden Check-in. "
                    "Entweder anschreiben oder bei EGYM nachmelden.", "warn")
        else:
            zeige = offen

        if zeige.empty:
            box("Nichts in dieser Kategorie.", "ok")
            return

        seitengroesse = 15
        if len(zeige) > seitengroesse:
            seiten = (len(zeige) - 1) // seitengroesse + 1
            seite = st.number_input(
                f"Seite (je {seitengroesse} Fälle)", min_value=1,
                max_value=seiten, value=1, step=1)
            zeige = zeige.iloc[(seite-1)*seitengroesse : seite*seitengroesse]
            st.caption(f"Seite {seite} von {seiten}")

        st.markdown("---")

        for i, (_, r) in enumerate(zeige.iterrows()):
            name = str(r["Name"])
            nn = str(r["Name_norm"])
            datum = str(r["Datum"])
            ...
            if vorschlaege:
                ...
                zeit_txt = (f" um <b>{ci_zeit}</b>" if ci_zeit else "")
                st.markdown(
                    f'<div class="pc-vorschlag">Bei EGYM hat '
                    f'<b>{bester}</b>{zeit_txt} eingecheckt — dieselbe Person? '
                    f'{chip(f"{score:.0f}%", sicherheit)} '
                    f'<span class="grund">Übereinstimmung: {grund}</span></div>',
                    unsafe_allow_html=True)
            ...
            if not alle_offen.empty:
                with st.expander("Anderen Namen zuordnen"):
                    ...
                    beschriftung = {}
                    for _, o in alle_offen.iterrows():
                        oz = str(o.get("Checkin_Zeit", "")).strip()
                        beschriftung[f"{o['Name']}" + (f"  ·  {oz} Uhr" if oz else "")] = \
                            str(o["Name_norm"])
                    auswahl = st.selectbox(
                        "Check-in", ["—"] + list(beschriftung.keys()),
                        key=f"wa_sel_{i}", label_visibility="collapsed")
                    if auswahl != "—":
                        ziel_norm = beschriftung[auswahl]
    ...
    # ── Überzählige Check-ins ───────────────────────────────────────────
    with t3:
        _wa_ueberzaehlig()

    # ── Erledigte Fälle ─────────────────────────────────────────────────
    with t4:
        box("Hier stehen alle Fälle, die du als erledigt markiert hast. "
...
def nachholung_speichern(checkin_datum: str, checkin_name: str,
    ...
    neu = pd.DataFrame([{
        "checkin_key": ck,
        "fall_key": checkin_schluessel(fall_datum, fall_name),
    ...
    savesheet(pd.concat([df, neu], ignore_index=True), "checkin_zuordnung")
    als_behoben_markieren(fall_name, fall_datum)
    loadsheet.clear()
