...
def _wa_seitenspalte(datum: str, offen_heute: pd.DataFrame):
    """Überzählige Check-ins der letzten Wochen — zum Zuordnen."""
    ...
    st.caption(f"letzte {rueck} Tage · eingecheckt, aber keiner Buchung "
               "zugeordnet")
    ...
    for _, r in ueber.iterrows():
        ...
        for label in ziele:
            ...
            sc = fuzz.token_set_ratio(normalize_name(klarname), ci_norm)
            mail = email_fuer(klarname)
            if mail:
                sc = max(sc, email_aehnlichkeit(mail, ci_name))
            bester = max(bester, sc)
        ci_d = parse_date_safe(str(r["analysis_date"]))
        naehe = abs((ci_d - heute).days) if ci_d else 99
        bewertung.append((bester, -naehe))
    ueber["_rang"] = [b[0] for b in bewertung]
    ueber["_naehe"] = [-b[1] for b in bewertung]
    ...
    for i, (_, r) in enumerate(ueber.head(30).iterrows()):
        ...
        # Bester Treffer unter den Fällen des Tages
        bester_key, bester_score = None, float(r.get("_rang", 0))
        for label in ziele:
            klarname = label.split("  ·  ")[0]
            sc = fuzz.token_set_ratio(normalize_name(klarname), ci_norm)
            mail = email_fuer(klarname)
            if mail:
                sc = max(sc, email_aehnlichkeit(mail, ci_name))
            if sc >= bester_score:
                bester_key, bester_score = label, sc
        ...
        if bester_score >= 80:
            if st.button(f"✓ = {bester_key.split('  ·  ')[0]}",
                         key=f"uez_q_{datum}_{i}", type="primary",
                         use_container_width=True):
...
def _wa_fall(r, i: int, datum: str):
    ...
    if vorschlaege:
        ...
        with v1:
            if st.button("✓ Ist dieselbe", key=f"wa_zu_{datum}_{i}",
                ...
                nachholung_speichern(datum, bester_norm, datum, nn)
                cache_leeren()
                st.success("Gemerkt.")
                time.sleep(.8)
                st.rerun()
    ...
    if nachhol:
        ...
        if st.button("✓ Nachgeholt — Fall schliessen",
                     key=f"wa_nh_{datum}_{i}", type="primary",
                     use_container_width=True):
            nachholung_speichern(cd, kn, datum, nn)
            cache_leeren()
...
def _wa_tagesarbeit():
    ...
    st.session_state.setdefault("wa_tag_idx", 0)
    st.session_state.wa_tag_idx = max(0, min(st.session_state.wa_tag_idx,
                                             len(tage) - 1))

    n1, n2, n3 = st.columns([1, 3, 1])
    with n1:
        if st.button("← Früher", use_container_width=True,
                     disabled=st.session_state.wa_tag_idx >= len(tage) - 1,
                     key="wa_frueher"):
            st.session_state.wa_tag_idx += 1
            st.rerun()
    with n3:
        if st.button("Später →", use_container_width=True,
                     disabled=st.session_state.wa_tag_idx <= 0,
                     key="wa_spaeter"):
            st.session_state.wa_tag_idx -= 1
            st.rerun()
    with n2:
        gewaehlt = st.selectbox(
            "Tag", tage, index=st.session_state.wa_tag_idx,
            format_func=lambda t: (datum_lang(t) +
                                   (f"  ·  {len(offene_fehler(t))} offen"
                                    if len(offene_fehler(t)) else "  ·  sauber")),
            label_visibility="collapsed", key="wa_tag_wahl")
        if gewaehlt != tage[st.session_state.wa_tag_idx]:
            st.session_state.wa_tag_idx = tage.index(gewaehlt)
            st.rerun()

    datum = tage[st.session_state.wa_tag_idx]
