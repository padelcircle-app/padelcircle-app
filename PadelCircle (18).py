...
def cache_leeren(*blaetter, funktionen=None):
    ...
    Ohne Angabe wird alles geleert — richtig nach einem Import.
    Nach einem einzelnen Klick reicht das betroffene Blatt: alles zu
    leeren bedeutet, dass jede folgende Anzeige sämtliche Tabellen neu
    von Google holt. Genau das machte jeden Knopfdruck langsam.
    """
    if blaetter:
        vollstaendig = False
        for blatt in blaetter:
            try:
                # Beide Aufrufvarianten treffen, sie haben eigene Cache-Einträge
                loadsheet.clear(blatt)
                spalten = SHEET_SPALTEN.get(blatt)
                if spalten is not None:
                    loadsheet.clear(blatt, spalten)
            except Exception:
                vollstaendig = True
                break
        if vollstaendig:
