...
def _verarbeiten(b_datei, c_datei, p_datei=None) -> bool:
    ...
    if kunden_out:
        ...
        if not alt.empty and "name_norm" in alt.columns:
            ...
            zusammen = alt.set_index("name_norm").to_dict("index")
        ...
        if k_datei and st.button("📤 Speichern", type="primary",
            ...
            if df.empty:
                ...
                if not alt_df.empty and "name_norm" in alt_df.columns:
                    zusammen = alt_df.set_index("name_norm").to_dict("index")
...
def _dash_abgleich():
    """Monatliche Gegenüberstellung: gegebene Rabatte vs. erhaltene Check-ins."""
    tage = verfuegbare_tage()
    if not tage:
        box("Noch keine Daten.", "warn")
        return

    box("Hier siehst du auf einen Blick, ob Rabatte und Check-ins zusammenpassen. "
        "Jede Lücke ist Geld, das EGYM dir nicht überweist, obwohl du den Court "
        "günstiger abgegeben hast.", "info")
    ...
    # ── Rabatte pro Buchung (nicht pro Zeile) zählen ────────────────────
    ...
    if "Wellpass_Rabatte" in mb.columns and schluessel:
        ...
        rabatte = 0
        box("Diese Daten stammen noch aus der Vorgängerversion und enthalten "
            "keine Rabatt-Information. Lade die betroffenen Tage in der "
            "Daten-Zentrale einmal neu hoch, dann stimmt der Abgleich.", "info")

    checkins = len(mc)
    luecke = rabatte - checkins
    offen = mb[mb["Fehler"].astype(str) == "Ja"]

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        kpi("Rabatte gegeben", str(rabatte), "laut Playtomic-Preisen")
    with c2:
        kpi("Check-ins erhalten", str(checkins), "laut EGYM")
    with c3:
        farbe_hinweis = "alles abgedeckt" if luecke <= 0 else "nicht gedeckt"
        kpi("Lücke", str(max(0, luecke)), farbe_hinweis)
    with c4:
        kpi("Geldwert der Lücke", euro(max(0, luecke) * WELLPASS_WERT),
            "entgangene Vergütung")

    st.markdown("")
    if luecke <= 0 and checkins > 0:
        box("✅ <b>Sauber.</b> Für jeden gegebenen Rabatt liegt ein Check-in vor"
            + (f" — sogar {abs(luecke)} mehr, das sind Spieler die eingecheckt "
               "haben ohne über Playtomic zu buchen." if luecke < 0 else "."), "ok")
    elif luecke > 0:
        box(f"⚠️ <b>{luecke} Rabatte ohne Check-in.</b> Das sind "
            f"{euro(luecke * WELLPASS_WERT)}, die dir für diesen Monat fehlen.", "warn")
    ...
    for t in monats_tage:
        ...
        chk = len(mc[mc["analysis_date"].astype(str) == t]) if not mc.empty else 0
        reihen.append({"tag": t[8:10], "Rabatte": rab, "Check-ins": chk})
    ...
    if reihen:
        ...
        fig.add_trace(go.Bar(x=df["tag"], y=df["Rabatte"], name="Rabatte gegeben",
                             marker_color=C["blue_soft"]))
        fig.add_trace(go.Bar(x=df["tag"], y=df["Check-ins"], name="Check-ins erhalten",
                             marker_color=C["volt"]))
        fig.update_layout(barmode="group")
        st.plotly_chart(plotly_layout(fig, 300, "Anzahl"), use_container_width=True)

    # ── Offene Fälle im Monat ───────────────────────────────────────────
    st.markdown("---")
    st.markdown("##### Wer konkret fehlt")
    if offen.empty:
        box("Keine offenen Einzelfälle.", "ok")
    else:
        spalten = [x for x in ["analysis_date", "Name", "Court", "Service_Zeit",
                               "Listenpreis", "Bezahlt", "Email"] if x in offen.columns]
        zeig = offen[spalten].copy()
        ...
        st.download_button(
            "⬇️ Monatsabgleich als CSV",
            data=zeig.to_csv(index=False, sep=";").encode("utf-8-sig"),
            file_name=f"abgleich_{monat}.csv",
            mime="text/csv", use_container_width=True)
    ...
    if not mc.empty and "Gespielt" in mc.columns:
        ...
        if not ohne.empty:
            st.markdown("---")
            st.markdown("##### Check-ins ohne passende Buchung")
            box(f"<b>{len(ohne)} Check-ins</b> konnten keiner Buchung zugeordnet "
                "werden. Meist steckt dahinter nur eine Namensvariante — schau "
                "im Modul <i>Name-Abgleich</i> nach. Bleibt jemand übrig, hat er "
                "gespielt ohne in Playtomic eingetragen zu sein.", "info")
            st.dataframe(ohne[["analysis_date", "Name", "Checkin_Zeit"]]
                         .rename(columns={"analysis_date": "Datum",
                                          "Checkin_Zeit": "Uhrzeit"}),
