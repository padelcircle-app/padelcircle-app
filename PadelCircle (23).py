...
def auto_zuordnungen_uebernehmen(tage: list = None,
    ...
    uebernommen = []
    for k in kandidaten:
        mapping_hinzufuegen(k["name_norm"], k["checkin_norm"], k["score"])
...
def _wa_auto_block(tage: list):
    """Sichere Zuordnungen anbieten und übernehmen."""
    if not einstellung("auto_zuordnung_an", True):
        return

    kandidaten = auto_kandidaten(tage)
    if not kandidaten:
        return

    schwelle = float(einstellung("auto_schwelle", AUTO_SCHWELLE_STANDARD))
    box(f"⚡ <b>{len(kandidaten)} Fälle sind eindeutig zuzuordnen</b> "
        f"(ab {schwelle:.0f} % und mit klarem Vorsprung). Das sind reine "
        "Schreibweisen-Unterschiede — der Check-in ist da und bezahlt.",
        "info")

    with st.expander(f"Die {len(kandidaten)} Zuordnungen ansehen"):
        ...
        st.dataframe(vorschau, use_container_width=True, hide_index=True,
                     height=min(320, 60 + 35 * len(kandidaten)))

    if st.button(f"⚡ Alle {len(kandidaten)} übernehmen und schliessen",
                 type="primary", use_container_width=True, key="auto_uebernehmen"):
        uebernommen = auto_zuordnungen_uebernehmen(tage)
        st.toast(f"{len(uebernommen)} Zuordnungen übernommen.")
        st.rerun()

    st.caption("Rückgängig unter Name-Abgleich → Gelernte Zuordnungen. "
...
def _wa_seitenspalte(datum: str, offen_heute: pd.DataFrame):
    ...
    for i, (_, r) in enumerate(ueber.head(30).iterrows()):
        ...
        if warnung:
            ...
            st.caption(f"Beleg: hat am {datum_kurz(ci_datum)} nicht gespielt")
...
def modul_einstellungen():
    ...
    with t2:
        ...
        if not vorschau:
            ...
            box(f"Bei {wert} % wären das aktuell <b>{len(vorschau)} Fälle</b>.",
                "ok")
