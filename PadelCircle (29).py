...
CONFIG = {
    ...
    "wellpass_abzug_alternativen": [13.00, 13.50, 11.00],
...
def wellpass_abzug() -> float:
    """Was Playtomic pro Wellpass-Spieler abzieht. Einstellbar."""
    try:
        return float(einstellung("wellpass_abzug", CONFIG["wellpass_abzug"]))
    except Exception:
        return float(CONFIG["wellpass_abzug"])


def wellpass_anzahl(liste: float, bezahlt: float, plaetze: int,
                    abzug_wert: float = None) -> int:
    """
    ...
    kandidaten = ([float(abzug_wert)] if abzug_wert
                  else [wellpass_abzug()] + list(
                      CONFIG["wellpass_abzug_alternativen"]))
...
def _zahlungs_index(pdf: pd.DataFrame) -> dict:
    ...
    for schluessel, gruppe in df.groupby(["_nn", "_dt"]):
        ...
        positiv = [x for x in betraege if x > 0]
        out[schluessel] = {
            "summe": float(sum(betraege)),
            "kleinste": min(positiv) if positiv else float(sum(betraege)),
            "zeilen": len(betraege),
        }
    return out
...
def _wellpass_traeger(teilnehmer, start, pro_platz, n_wellpass, zahlungen) -> set:
    ...
    mitzahler = set()       # hat mehr als den eigenen Platz bezahlt
    ...
    for nn in namen:
        ...
        if daten["summe"] > pro_platz + 1.0:
            # Er hat für mehr als seinen eigenen Platz gezahlt — also für
            # jemanden mitbezahlt, der deshalb gar keine eigene Zeile hat.
            mitzahler.add(nn)

        # Entscheidend ist die kleinste Einzelzahlung, nicht die Summe:
        # sie zeigt, was er für seinen eigenen Platz gezahlt hat.
        if pro_platz - daten["kleinste"] > 1.0:
            sicher.add(nn)
        else:
            voll.add(nn)
    ...
    #
    # Nur zulässig, wenn niemand für andere mitbezahlt hat. Sonst wurde die
    # offene Person schlicht eingeladen — dann steht sie zu Recht in keiner
    # Zahlungszeile und ist erst recht kein Rabattträger.
    if (len(unbekannt) == 1 and len(sicher) == n_wellpass - 1
            and not mitzahler):
        return sicher | unbekannt

    if mitzahler:
        # Hat einer die ganze Buchung gezahlt und es gab trotzdem einen
        # Rabatt, dann steckt der in seinem eigenen Platz. Die Eingeladenen
        # haben Vollpreis gekostet.
        if sicher:
            return sicher
        return mitzahler
...
def modul_einstellungen():
    ...
    with t2:
        ...
        abzug = st.number_input(
            "Abzug pro Wellpass-Spieler in €", min_value=5.0, max_value=25.0,
            value=float(wellpass_abzug()), step=0.5, format="%.2f",
            key="abzug_regler")
        if st.button("Abzug speichern", type="primary", key="abzug_speichern"):
            einstellung_setzen("wellpass_abzug", float(abzug))
            cache_leeren()
