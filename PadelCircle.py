...
ERLEDIGT_GRUENDE = {
    ...
    "paypal": {
        "label":   f"Per PayPal bezahlt ({ADMIN_GEBUEHR:.0f} €)",
        "kurz":    "PayPal bezahlt",
        "icon":    "💶",
        "art":     "einnahme",
        "wert":    ADMIN_GEBUEHR,
        "hilfe":   "Bearbeitungsgebühr nach deiner Nachricht überwiesen.",
    },
    "nachgeholt": {
...
SHEET_SPALTEN = {
    ...
    "checkins":         None,
    "customers":        None,
    "corrections":      ["key", "date", "behoben", "grund", "notiz", "timestamp"],
...
def cache_leeren():
    ...
    for fn_name in ("tages_kennzahlen", "verfuegbare_tage", "monats_kennzahlen",
                    ...
                    "einstellungen_laden",
                    # ── Caches der Beschleunigung ──
                    "kontakt_index", "offene_fehler", "offene_je_tag",
                    "zuordnung_vorschlag", "nachhol_kandidaten",
                    "offene_checkins", "offene_checkins_zeitraum",
                    "verbrauchte_checkins"):
...
def offene_je_tag() -> dict:
    ...
    return (fehler.groupby(fehler["analysis_date"].astype(str))
...
def als_behoben_markieren(name_norm: str, datum: str,
                          grund: str = "", notiz: str = ""):
    """
    Einen Fall schliessen — mit Grund, weil die vier Fälle
    wirtschaftlich völlig verschieden sind.
    """
    corr = loadsheet("corrections", SHEET_SPALTEN["corrections"])
...
def erledigte_faelle() -> pd.DataFrame:
    ...
    df["grund_wert"] = df["grund"].map(lambda g: grund_info(g)["wert"])
...
def erledigt_bilanz(monat: str = None) -> dict:
    ...
    for schluessel in list(ERLEDIGT_GRUENDE.keys()) + [""]:
        ...
        je_grund[schluessel or "ohne_grund"] = {
            "anzahl": int(len(teil)),
            "wert": round(len(teil) * info["wert"], 2),
            "label": info["label"],
            "kurz": info["kurz"],
            "icon": info["icon"],
            "art": info["art"],
...
def grund_dialog(kennung: str, name: str, name_norm: str, datum: str):
    ...
    if info["art"] == "kein_fall":
        ...
        wirkung = f"+ {euro(info['wert'])}"
    else:
        wirkung = f"− {euro(abs(info['wert']))} echter Verlust"
    st.caption(f"{info['hilfe']}  ·  **{wirkung}**")

    notiz = st.text_input("Notiz (optional)", key=f"grund_notiz_{kennung}",
                          placeholder="z. B. PayPal am 12.07. eingegangen")

    g1, g2 = st.columns(2)
    with g1:
        if st.button("Fall schliessen", key=f"grund_ok_{kennung}",
                     type="primary", use_container_width=True):
            als_behoben_markieren(name_norm, datum, wahl, notiz)
...
def _dash_abgleich():
    ...
    if bil["anzahl"] == 0:
        ...
        with st.expander("Einzelne Fälle ansehen"):
            det = bil["df"][["datum", "Name", "grund_label", "notiz"]].copy()
            det.columns = ["Spieltag", "Spieler", "Grund", "Notiz"]
...
def _wa_tagesarbeit():
    ...
    with n2:
        ...
        datum = st.selectbox("Tag", tage, format_func=_tag_label,
