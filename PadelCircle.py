...
APP_STAND       = "Fassung 39 · 16.09.2026"
...
def _wa_seitenspalte(datum: str, offen_heute: pd.DataFrame):
    ...
    n_geschenke = 0
    for k in geschenk_checkins():
        d = parse_date_safe(str(k).split("|", 1)[0])
        if d is not None and von <= d <= bis:
            n_geschenke += 1
    if n_geschenke:
        st.caption(f"{n_geschenke} Check-in{'s' if n_geschenke != 1 else ''} in "
                   "diesem Zeitraum als Geschenk abgelegt — voll bezahlt "
                   "gespielt, nichts zuzuordnen. Nachsehen und zurücknehmen "
                   "unter Übersicht.")
