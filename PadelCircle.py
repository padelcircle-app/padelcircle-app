...
def dubletten_bereinigen(sheet_name: str) -> tuple:
    """
    Vorhandene Dubletten aus einem Blatt entfernen.

    Nötig für Daten, die vor der Hash-Prüfung hochgeladen wurden.
    → (vorher, nachher)
    """
    ...
    vorher = len(df)
    arbeit = df.copy()
    spalten = [c for c in arbeit.columns if c != "_hash"]
    arbeit["_hash"] = _zeilen_hash(arbeit, spalten)
    arbeit = arbeit.drop_duplicates(subset=["_hash"])
    nachher = len(arbeit)
    if nachher < vorher:
        ...
        if roh_p.empty:
            ...
            if st.button("🧹 Jetzt bereinigen", type="primary",
                ...
                with st.spinner("Prüfe alle Datenblätter…"):
                    ergebnis = []
                    for blatt in ("playtomic_raw", "buchungen", "checkins"):
                        vor, nach = dubletten_bereinigen(blatt)
                        if vor != nach:
