...
def _wellpass_traeger(teilnehmer, start, pro_platz, n_wellpass, zahlungen) -> set:
    ...
    Grundlage sind allein die Zahlungen — nur dort steht, was tatsächlich
    geflossen ist. Für jede Zahlung wird zurückgerechnet, aus wie vielen
    vollen und wie vielen rabattierten Plätzen sie besteht.

    Daraus folgt unmittelbar:
      • Wer nur volle Plätze bezahlt hat, hatte keinen Rabatt — auch
        dann nicht, wenn er für mehrere Leute gezahlt hat.
      • Wer rabattierte Plätze bezahlt hat, hatte einen.
      • Bleiben danach noch Rabatte übrig, gehören sie zu Mitspielern
        ohne eigene Zahlungszeile.

    Die Regel gilt für jede Buchung gleichermassen; es gibt keine
    ...
    # Welcher Rabattbetrag steckt in dieser Buchung?
    #
    # Der Wert des Spieltags zuerst, dann die anderen bekannten. Nötig,
    # weil bis zu zwei Wochen im Voraus gebucht wird: nach einer
    # Umstellung laufen noch länger Buchungen mit dem alten Rabatt ein.
    ...
    for abzug in kandidaten:
        ...
        sicher, unklar, erklaerte = set(), set(), 0

        for nn, daten in mit_zeile.items():
            werte = daten.get("werte") or [daten["summe"]]
            rabatte, lesbar = 0, True
            for betrag in werte:
            ...
            if not lesbar:
                ...
                sicher.add(nn)
                erklaerte += rabatte
            # sonst: nur volle Plätze bezahlt — kein Rabattträger

        # Alle Zahlungen lesbar und die Rabattzahl geht auf → fertig
        if not unklar and erklaerte == n_wellpass and len(sicher) == n_wellpass:
            return sicher

        if bestes is None or len(unklar) < len(bestes[1]):
            bestes = (sicher, unklar, erklaerte)

    sicher, unklar, erklaerte = bestes

    # Keine eindeutige Auflösung: Wer nachweislich nur Vollpreis gezahlt
    # hat, bleibt draussen. In Frage kommen die Rabattzahler, die
    # unlesbaren Zahlungen und alle ohne eigene Zeile — Letztere wurden
    # von jemandem mitbezahlt und können den Rabatt getragen haben.
    offen = sicher | ohne_zeile | unklar
    return offen or set(namen)
