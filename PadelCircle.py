...
APP_STAND       = "Fassung 34 · 15.09.2026"
...
def _platz_zerlegen(betrag: float, anteile, einzel, kandidaten: list,
    ...
    Sind beide möglich, entscheidet der Beleg aus demselben Slot: Haben
    ANDERE Spieler zur selben Zeit genau diesen Rabattpreis oder genau
    diesen vollen Anteil gezahlt, gilt diese Lesart. Jenny und marc
    zahlten um 18:30 je 1,50 € — also ist 3,00 € zweimal 1,50 €, nicht
    einmal 3,00 €. Bei einem fremden Abzug (13 € statt 12 €) zählt für
    die Mehrfach-Lesart nur der Rabattpreis selbst als Beleg. Ohne
    Beleg gewinnt der Abzug des Spieltags, dann die Ein-Platz-Lesart.
    ...
    for kand in kandidaten:
        ...
        if b <= ANTEIL_MAX - kand:
            voll = round(b + kand, 2)
            if voll in anteile:
                beleg = (voll in andere) or (b in andere)
                lesarten.append(((beleg, eigener_satz, True), 1, voll, b))
        ...
        for voll in einzel:
            ...
            if abs(n - round(n)) > 0.01 or not (2 <= round(n) <= spieler):
                continue
            beleg = (r in andere) or (eigener_satz and voll in andere)
            lesarten.append(((beleg, eigener_satz, False),
                             int(round(n)), voll, r))
...
def _analysieren_zahlungen(pdf, cdf, bdf=None, tage_ersetzen=None) -> bool:
    ...
    for i, (g, _txt) in enumerate(ansprueche):
        ...
        # 2 · Ohne Buchungsdatei bleiben die Check-ins des Tages.
...
def _export_buchungen_index(bdf) -> dict:
    """
    {(tag, zeit): [[(Name, name_norm), …] je Buchung]} — ohne stornierte
    Buchungen und ohne Platzhalter wie „Player 2".
    """
    idx = {}
    if bdf is None or getattr(bdf, "empty", True) \
    ...
    for _, r in bdf.iterrows():
        ...
        for i in range(1, 21):
            ...
            if n and n.lower() != "nan" and not ist_platzhalter(n):
                leute.append((n, normalize_name(n)))
        if leute:
