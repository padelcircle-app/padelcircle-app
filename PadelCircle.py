...
APP_STAND       = "Fassung 40 · 16.09.2026"
...
def offene_checkins(datum_str: str) -> pd.DataFrame:
    ...
    # Geschenke bleiben draussen: Ein Check-in, der zu einem voll
    # bezahlten Spiel gehört, kann keinen Fall erklären — weder als
    # Schreibvariante noch als Nachholung.
    offen = _ohne_geschenke(offen)

    return offen.drop_duplicates(subset=["Name_norm"])


def _ohne_geschenke(df: pd.DataFrame) -> pd.DataFrame:
    """Als Geschenk abgelegte Check-ins und die von Familie/Team entfernen."""
    if df.empty or not {"analysis_date", "Name_norm"} <= set(df.columns):
        return df
    geschenke = geschenk_checkins()
    maske = [checkin_schluessel(str(t), str(n)) not in geschenke
             and str(n) not in GESCHENK_NORM
             for t, n in zip(df["analysis_date"], df["Name_norm"])]
    return df[maske]
...
def _passenden_checkin(g: dict, checkins: list, nur_gleicher_name: bool,
    ...
    for c in checkins:
        ...
        if (namen_sind_verschiedene_personen(g["name_norm"], c["name_norm"])
                or namen_unvereinbar(g["name_norm"], c["name_norm"])):
            continue
...
def _analysieren_zahlungen(pdf, cdf, bdf=None, tage_ersetzen=None) -> bool:
    ...
    umbenannt = _namen_aus_buchung(slots, bdf)
    if umbenannt:
    ...
    for g, art, _t, voll, _a in bewertet:
        ...
        in_buchung = any(
            nn_ == g["name_norm"] or namen_decken_sich(nn_, g["name_norm"])
            for leute in export_bu.get((str(g["datum"]), g["zeit"]), [])
            for _n, nn_, _m in leute)
        if in_buchung:
            continue
    ...
    for i, (g, _txt) in enumerate(ansprueche):
        ...
        gefunden = None
        for leute in export_bu.get((str(g["datum"]), g["zeit"]), []):
            if not any(namen_decken_sich(nn_, g.get("zahler", ""))
                       for _n, nn_, _m in leute):
                continue
            ohne = [(n, nn_) for n, nn_, _m in leute
                    if not any(namen_decken_sich(ag["name_norm"], nn_)
    ...
    # Check-ins von Leuten, die voll bezahlt gespielt haben, sind
    # Geschenke — die legt die App jetzt selbst ab.
    geschenke_neu = geschenke_automatisch(
        {b["analysis_date"] for b in buchungen_out}
        | {c["analysis_date"] for c in checkins_out}, bdf)
    if geschenke_neu:
        st.caption(f"{geschenke_neu} Check-in{'s' if geschenke_neu != 1 else ''} "
                   "als Geschenk abgelegt — voll bezahlt gespielt, nichts "
                   "zuzuordnen.")
...
def _namen_aus_buchung(slots: list, bdf) -> int:
    """
    Zahler, die in der Buchung unter ihrem vollen Namen stehen, bekommen
    diesen Namen. → Anzahl umbenannter Zahler
    ...
    checkte „Jennifer Kries" um 18:37 ein. „Jenny" und „Jennifer" sind
    für keine Ähnlichkeitsregel dasselbe Wort. Die Buchung sagt es
    trotzdem eindeutig: Drei der vier Teilnehmer haben ihre Zahlung,
    übrig bleibt genau eine Teilnehmerin und genau eine Zahlerin.

    Dazu kommt: Es gibt ZWEI Playtomic-Konten namens „Jenny" (IDs 9146648
    und 9046526). Eine gemerkte Namensverknüpfung „Jenny → …" träfe beide.
    Der Name aus der Buchung gilt nur für diese eine Zahlung in diesem
    einen Slot — genau richtig.

    Umbenannt wird nur bei glatter Rechnung: im Slot genau ein Zahler
    ohne Teilnehmer und über alle Buchungen des Slots genau ein
    Teilnehmer ohne Zahlung, und die beiden Namen widersprechen sich
    nicht (Kurzform oder derselbe Anfang). Wer den ganzen Platz allein
    zahlt, bleibt aussen vor — der steht manchmal gar nicht in der
    Teilnehmerliste.
    """
    ...
    for k, buchungen in idx.items():
        ...
        if not zahler:
            continue
        vergeben, offen_teilnehmer = set(), []
        for leute in buchungen:
            for n, nn, mail in leute:
                treffer = next((g for g in zahler if id(g) not in vergeben
                                and g["name_norm"] == nn), None)
                if treffer is None:
                    treffer = next((g for g in zahler if id(g) not in vergeben
                                    and namen_decken_sich(g["name_norm"], nn)
                                    and not namen_unvereinbar(g["name_norm"], nn)),
                                   None)
                if treffer is None:
                    offen_teilnehmer.append((n, nn, mail))
                else:
                    vergeben.add(id(treffer))
        offen_zahler = [g for g in zahler if id(g) not in vergeben
                        and not g.get("einzelzahler")]
        if len(offen_zahler) != 1 or len(offen_teilnehmer) != 1:
            continue
        g = offen_zahler[0]
        n, nn, mail = offen_teilnehmer[0]
        z = g["name_norm"]
        vertraeglich = (bool(z) and not namen_unvereinbar(z, nn)
                        and any(t[:2] == z[:2] or t.startswith(z) or z.startswith(t)
                                for t in nn.split() if t))
        if not vertraeglich:
            continue
        g["name_zahlung"] = g["name"]
...
def geschenke_automatisch(tage, bdf=None) -> int:
    """
    Check-ins, die nachweislich zu einem voll bezahlten Spiel gehören,
    von selbst als Geschenk ablegen. → Anzahl

    Jürgen Friederich am 02.09.: gespielt, voll bezahlt, trotzdem
    eingecheckt. EGYM vergütet, zuzuordnen gibt es nichts — und Marcel
    musste das bisher selbst erkennen und anklicken. Die Dateien sagen
    es aber eindeutig: eine eigene Zahlung ohne Rabatt an dem Tag, oder
    laut Buchungsdatei Teilnehmer einer Buchung ohne Rabatt.

    Zwei Sicherungen:
      • Hat die Person an dem Tag irgendwo einen Anspruch — auch unter
        einer Schreibvariante —, ist der Check-in kein Geschenk, sondern
        gehört dorthin.
      • Hat sie in den Tagen davor einen offenen Fall, bleibt der
        Check-in stehen: Er könnte den nachholen.
    """
    vollzahler = _export_vollzahler(bdf) if bdf is not None else {}
    b = loadsheet("buchungen")
    markiert = geschenk_checkins()
    fenster = fenster_nachhol()
    alle_tage = verfuegbare_tage()

    def _passt(nn, namen) -> bool:
        return any(nn == x or (namen_decken_sich(nn, x)
                               and not namen_unvereinbar(nn, x))
                   for x in namen)

    neu = []
    for tag in sorted({str(t) for t in tage}):
        ueber = offene_checkins(tag)
        if ueber.empty:
            continue
        d = parse_date_safe(tag)
        if d is None:
            continue
        davor = [t for t in alle_tage
                 if parse_date_safe(t) is not None
                 and d - timedelta(days=fenster) <= parse_date_safe(t) < d]
        offen_davor = alle_offenen_fehler(davor) if davor else pd.DataFrame()
        offen_namen = (set(offen_davor["Name_norm"].astype(str))
                       if not offen_davor.empty else set())

        tag_b = (b[b["analysis_date"].astype(str) == tag]
                 if not b.empty and "analysis_date" in b.columns else pd.DataFrame())
        if not tag_b.empty and "Relevant" in tag_b.columns:
            rabatt = set(tag_b.loc[tag_b["Relevant"].astype(str) == "Ja",
                                   "Name_norm"].astype(str))
            ohne_rabatt = set(tag_b.loc[tag_b["Relevant"].astype(str) != "Ja",
                                        "Name_norm"].astype(str))
        else:
            rabatt, ohne_rabatt = set(), set()
        voll = list(vollzahler.get(tag, []))

        for _, r in ueber.iterrows():
            nn = str(r["Name_norm"])
            key = checkin_schluessel(tag, nn)
            if key in markiert or nn in GESCHENK_NORM:
                continue
            if _passt(nn, rabatt):
                continue                    # hat einen Anspruch — gehört dorthin
            if not (_passt(nn, ohne_rabatt) or _passt(nn, voll)):
                continue                    # kein Beleg fürs Spielen ohne Rabatt
            if _passt(nn, offen_namen):
                continue                    # könnte einen älteren Fall nachholen
            neu.append({
                "checkin_key": key, "datum": tag, "name": str(r["Name"]),
                "grund": "voll bezahlt gespielt — automatisch erkannt",
                "timestamp": datetime.now().isoformat(timespec="seconds"),
            })
    ...
    if not neu:
        return 0
    df = loadsheet("geschenke", SHEET_SPALTEN["geschenke"])
    zusammen = (pd.concat([df, pd.DataFrame(neu)], ignore_index=True)
                if not df.empty else pd.DataFrame(neu))
    if savesheet(zusammen, "geschenke"):
        cache_leeren("geschenke", funktionen=(
            "geschenk_checkins", "offene_checkins", "offene_checkins_zeitraum",
            "zuordnung_vorschlag", "nachhol_kandidaten",
            "_auto_kandidaten_gerechnet", "checkin_erklaerung"))
        return len(neu)
    return 0
