...
def _wellpass_traeger(teilnehmer, start, pro_platz, n_wellpass, zahlungen,
    ...
    for abzug in kandidaten:
        ...
        for paar in paare:
            ...
            if rabatte >= len(paar) - len(unlesbar):
                traeger.update(nn for nn in paar if nn not in unlesbar)
                continue

            # Teilweise rabattiert — wer von beiden?
            eingecheckt = [nn for nn in paar if nn in checkin_namen]
            mitglieder = [nn for nn in paar if nn in wellpass_bekannt]
            selbst_rabattiert = [nn for nn, (_, m) in eigene.items() if m > 0]

            if len(eigene) == len(paar):
                # Jeder hat selbst gezahlt → die eigene Zahlung entscheidet
                traeger.update(selbst_rabattiert)
            elif len(eingecheckt) == rabatte:
                traeger.update(eingecheckt)          # Check-in an dem Tag
            elif len(mitglieder) == rabatte:
                traeger.update(mitglieder)           # bekannte Mitglieder
            else:
                offen_paare.append([nn for nn in paar if nn not in unlesbar])
