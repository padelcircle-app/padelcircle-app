...
def zuordnung_zu_fall_loesen(name_norm: str, datum: str) -> bool:
    ...
    if not (df["fall_key"].astype(str) == fk).any():
        return False
...
def _zahlungs_index(pdf: pd.DataFrame) -> dict:
    ...
    Echtes Beispiel: Julian Keidler zahlt 16,50 € für seinen Gast und
    4,50 € für sich mit Wellpass. Summiert ergibt das 21 € — und die
    App hielt daraufhin den Gast für den Rabattträger.
    ...
    for schluessel, gruppe in df.groupby(["_nn", "_dt"]):
        ...
        # Nur echte Rückerstattungen ausblenden. Eine 0,00-€-Zeile ist
        # keine Rückerstattung, sondern der Beweis für einen vollen
        # Wellpass-Rabatt — die darf nicht verloren gehen.
        ohne_rueck = [x for x in betraege if x >= 0]
        out[schluessel] = {
            "summe": float(sum(betraege)),
            "kleinste": min(ohne_rueck) if ohne_rueck else float(sum(betraege)),
            "zeilen": len(betraege),
        }
    return out
...
def _wellpass_traeger(teilnehmer, start, pro_platz, n_wellpass, zahlungen) -> set:
    ...
    Die Zahlungen sind hier die Wahrheit, nicht der Buchungspreis: Nur
    dort steht, was jeder Einzelne gezahlt hat.

    Drei Muster, die im Betrieb wirklich vorkommen:

    1. Jeder zahlt selbst → wer unter dem Platzpreis liegt, hatte den
       Rabatt.
    2. Einer lädt jemanden ein → der Eingeladene hat gar keine eigene
       Zeile. Er hat trotzdem VOLLEN Preis gekostet, denn ein Wellpass
       hängt am eigenen Konto und hätte die Zahlung des Zahlers
       gesenkt. Er ist deshalb nie der Träger.
    3. Der Zahler bündelt beides in einer Zeile — Benjamin zahlte
       14,00 €: 0,50 € für sich mit Wellpass und 13,50 € für Steffen.
       Dann muss der Fremdanteil abgezogen werden, um seinen eigenen
       Platz zu sehen.

    Ohne Punkt 2 und 3 wurden reihenweise Gäste angeschrieben, die nie
    einen Rabatt hatten.
    """
    # Team zahlt 0 €, Platzhalter sind keine Menschen — beide scheiden aus
    namen = [normalize_name(n) for n, _ in teilnehmer
             if normalize_name(n) not in TEAM_NORM and not ist_platzhalter(n)]
    if n_wellpass <= 0 or not namen:
        return set()
    if not zahlungen or n_wellpass >= len(namen):
        return set(namen)

    mit_zeile, ohne_zeile = {}, set()
    for nn in namen:
    ...
    # Wer hat mehr als seinen eigenen Platz gezahlt?
    mitzahler = [nn for nn, d in mit_zeile.items()
                 if d["summe"] > pro_platz + 0.01]

    # Eigenanteil je Person bestimmen
    eigenanteil = {}
    for nn, d in mit_zeile.items():
        eigen = d["kleinste"]
        # Gibt es genau einen Sammelzahler und Personen ohne eigene
        # Zeile, trägt er deren Plätze. Zum Vollpreis, denn ein Rabatt
        # hätte an ihrem Konto gehangen.
        if len(mitzahler) == 1 and nn == mitzahler[0] and ohne_zeile:
            rest = d["summe"] - pro_platz * len(ohne_zeile)
            if -0.01 <= rest < eigen:
                eigen = max(0.0, rest)
        eigenanteil[nn] = eigen

    sicher = {nn for nn, e in eigenanteil.items() if pro_platz - e > 1.0}

    if len(sicher) == n_wellpass:
        return sicher

    # Wer von jemandem mitbezahlt wurde, hat Vollpreis gekostet und
    # scheidet als Träger aus — auch wenn dann nicht alle Rabatte
    # zugeordnet sind.
    if ohne_zeile and mitzahler:
        return sicher or set(mit_zeile)

    # Ausschlussverfahren: genau eine offene Person, genau ein offener
    # Rabatt — und niemand hat für andere mitbezahlt.
    if (len(ohne_zeile) == 1 and len(sicher) == n_wellpass - 1
            and not mitzahler):
        return sicher | ohne_zeile
...
def _dash_einnahmen():
    ...
    with e2:
        kpi("Wellpass", euro(wellpass),
            f"{k['wellpass_anzahl']} Check-ins × {euro(wellpass_wert_am(f'{monat}-01'))}")
    with e3:
